...

{
    // Create scaling object, to be used for each partial created here (i.e. same scaling for different parameters but same
    // observable).
    std::shared_ptr< VelocityObservationScaling > velocityObservableScaling =
            std::make_shared< VelocityObservationScaling >( );

    SingleLinkObservationThreePartialList velocityObservablePartials;
    ...
    for( unsigned int i = 0; i < initialDynamicalParameters.size( ); i++ )
    {
                    ...
                    initialDynamicalParameters.at( i ) ) != NULL )
        {
            ...
            // Retrieve name of body w.r.t. velocity of which partial is to be taken.
            ...
            {
                currentPair = std::pair< int, int >( currentIndex, 6 );
                velocityObservablePartials[ currentPair ] = currentObservablePartial;
            }
    ...
    return std::make_pair( velocityObservablePartials, velocityObservableScaling );
}
...
{
    std::map< observation_models::LinkEnds, std::pair< SingleLinkObservationThreePartialList,
            std::shared_ptr< PositionPartialScaling > > > velocityObservablePartials;

    for( unsigned int i = 0; i < linkEnds.size( ); i++ )
    {
        if( linkEnds[ i ].count( observation_models::observed_body ) == 0 || linkEnds[ i ].size( ) != 1 )
        {
            throw std::runtime_error( "Error when making velocity observable partial, link ends are wrong" );
        }
